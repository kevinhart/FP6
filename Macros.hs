...
data Macro a = Macro { macroKey :: String, macroValue :: SExpr a }
...
substitute (x:xs) exp = substitute xs (sub x exp)
  where
    sub mac (Name a) = if True then (macroValue mac) else (Name a)
...
showsMacro x = ("(define "++) . ((macroKey x)++) . (" "++) . showsSExpr (macroValue x) . (')':)
